...
num_labels = 64
embedding_dim = 16  # 埋め込みベクトルの次元

train_model = True
save_model = True

project_name = "sample08"
...
class ColorDecoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(encoder_output_channels + embedding_dim, 16, 3, stride=2, padding=1, output_padding=1),
            nn.ReLU(),
    ...
    if train_model:
        ...
        for epoch in range(num_epochs):
            for label_index in range(64):
                ...
                label_emb = label_emb.view(1, embedding_dim, 1, 1).expand(1, embedding_dim, image_size//4, image_size//4)

                # 順伝播
                encoder_output = encoder(image[:, :, label_index])
