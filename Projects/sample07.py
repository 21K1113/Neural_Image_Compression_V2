...
if train_model:
    ...
    torch.save(encoder.state_dict(), f'model/image_{device}_{basename}_{num_epochs}_encoder.pth')
    torch.save(decoder.state_dict(), f'model/image_{device}_{basename}_{num_epochs}_decoder.pth')

else:
    # デコーダの訓練済みパラメータのロード
    basename = os.path.basename(image_path)
    decoder.load_state_dict(torch.load(f'model/image_{device}_{basename}_{num_epochs}_decoder.pth'))
    decoder.eval()
...
if save_model:
    if not train_model:
        # エンコーダの訓練済みパラメータのロード
        basename = os.path.basename(image_path)
        encoder.load_state_dict(torch.load(f'model/image_{device}_{basename}_{num_epochs}_encoder.pth'))
        encoder.eval()
    ...
    basename = os.path.basename(image_path)
    np.save(f'comp/image_{device}_{basename}_{num_epochs}.npy', compressed_np)

else:
    # 圧縮された潜在表現を読み込み
    basename = os.path.basename(image_path)
    compressed_np = np.load(f'comp/image_{device}_{basename}_{num_epochs}.npy')
...
reconstructed_image = reconstructed.squeeze().permute(1, 2, 0).cpu().numpy() * 255
reconstructed_image = reconstructed_image.astype(np.uint8)
reconstructed_image = Image.fromarray(reconstructed_image)
reconstructed_image.save(f'image/image_{device}_{basename}_{num_epochs}.png')
