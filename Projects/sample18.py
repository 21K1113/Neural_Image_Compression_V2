...
def random_crop_dataset(datasets, crop_size, num_crops):
    ...
    crop_height, crop_width = crop_height // pow(2, lod), crop_width // pow(2, lod)
...
class RandomCropDataset(Dataset):
    def __init__(self, dataset, crop_size, num_crops):
        ...
        self.crops, self.coord, self.lod = self.get_random_crops()
    ...
    def __getitem__(self, idx):
        self.crops, self.coord, self.lod = self.get_random_crops()
        return self.crops[idx], self.coord[idx], self.lod
...
def train_models(fp):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, coord, lod in dataloader:
            fl = feature_pyramid_mip_levels_dict[lod]
...
random_crop_dataset = RandomCropDataset(images, crop_size, num_crops)
...
psnr = calculate_psnr(image.cpu().numpy().squeeze().transpose(1, 2, 0).astype(np.float32),
